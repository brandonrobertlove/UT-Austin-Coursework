...
class WeatherForecast:
    ...
    def find_min_and_max_per_day(self) -> Tuple[torch.Tensor]:
        ...
        """
        return torch.squeeze(torch.min(self.data, dim=1)[0].view(-1,1)), torch.squeeze(torch.max(self.data, dim=1)[0].view(-1, 1))
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        """
        return (torch.mean(self.data, dim=1)[1:] - torch.mean(self.data, dim=1)[:-1])[torch.argmin(torch.mean(self.data, dim=1)[1:] - torch.mean(self.data, dim=1)[:-1])]
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        return torch.squeeze(torch.gather(self.data, 1, torch.argmax(torch.abs(self.data - torch.mean(self.data, dim=1, keepdim=True)), dim=1).unsqueeze(1)))
    ...
    def max_last_k_days(self, k: int) -> torch.Tensor:
        ...
        Returns:
            tensor of size (k,)
        """
        return torch.squeeze(torch.max(self.data[-k:], dim=1)[0].view(-1, 1), dim=1)
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        """
        return torch.Tensor.long(torch.argmin(torch.cumsum(torch.abs(self.data - t), dim=1)[:, -1]))
