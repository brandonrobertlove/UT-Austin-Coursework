...
class NearestNeighborClassifier:
    ...
    def compute_data_statistics(cls, x: torch.Tensor) -> tuple[torch.Tensor]:
        ...
        """
        return (x.mean(dim=0).unsqueeze(1).view(1,-1), x.std(dim=0).unsqueeze(1).view(1, -1))
    ...
    def get_k_nearest_neighbor(self, x: torch.Tensor, k: int) -> tuple[torch.Tensor]:
        ...
        dist = torch.as_tensor([((norm_data - x).pow(2).sum()) for norm_data \
                                 in self.data_normalized])
        sorted, idx = torch.sort(dist)
        #idx, xxxxx = torch.topk(indices, k)
        return (self.data[idx[:k]], self.label[idx][:k])
    ...
    def knn_regression(
        ...
        """
        nearest_neighbors_data, nearest_neighbors_labels = self.get_k_nearest_neighbor(x, k)

        return torch.mean(nearest_neighbors_labels)
