...
def predict_two_classes(examples: List[FactExample], fact_checker):
    ...
    for i, example in enumerate(tqdm(examples)):
        ...
        if gold_label != pred_label:
            ...
            if gold_label == 0 and pred_label == 1:  # False Negative
                false_negatives.append(false_case)
            elif gold_label == 1 and pred_label == 0:  # False Positive
                false_positives.append(false_case)

    print_eval_stats(confusion_mat, gold_label_indexer)
    log_false_cases(false_positives, "false_positives")
    log_false_cases(false_negatives, "false_negatives")


def log_false_cases(false_cases, case_type):
    """
    Logs False Positives or False Negatives to a text file with their predicted and true labels.
    :param false_cases: list of dictionaries containing the fact, predicted label, true label, and passages
    :param case_type: a string indicating whether the log is for false positives or false negatives
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_filename = f"{case_type}_log_{timestamp}.txt"
    # Use UTF-8 encoding to handle any Unicode characters
    with open(log_filename, "w", encoding="utf-8") as log_file:
        log_file.write(f"{case_type.replace('_', ' ').title()}:\n\n")
        for case in false_cases:
            log_file.write(f"Fact: {case['fact']}\n")
            log_file.write(f"Predicted Label: {case['predicted_label']}\n")
            log_file.write(f"True Label: {case['true_label']}\n")
            log_file.write(f"Passages:\n")
            for passage in case['passages']:
                log_file.write(f"  - {passage['text']}\n")
            log_file.write("\n")
    print(f"{case_type.replace('_', ' ').title()} log saved to {log_filename}")
